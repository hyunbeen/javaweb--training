...
public class BoardDao
{
	...
	private static final String		dbUrl		=	"jdbc:oracle:thin:@192.168.0.156:1521:orcl";
	private static final String		dbUser		=	"been";
	private static final String		dbPass		=	"love1998";
	...
	public int getGroupId() throws BoardException
	{
		...
		try{
			...
			String sql = "select seq_group_id_article.nextval seq_group_id  from dual";
			
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			if(rs.next()) {
				groupId = Integer.parseInt(rs.getString("seq_group_id"));
			}
			return groupId;
		}catch( Exception ex ){
	...
	public List<BoardRec> selectList(int firstrow,int endrow) throws BoardException
	{
		...
		try{
			con = DriverManager.getConnection(dbUrl, dbUser, dbPass);
			String sql = "select article_id article_id,title title,writer_name writer_name,posting_date posting_date,read_count read_count,group_id group_id,sequence_no sequence_no\r\n" + 
					"from(select rownum rnum ,article_id article_id,title title,writer_name writer_name,posting_date posting_date,read_count read_count,group_id group_id,sequence_no sequence_no\r\n" + 
					"from(select * from article order by sequence_no desc))\r\n" + 
			...
			while(rs.next()) {
				...
				b.setReadCount(Integer.parseInt(rs.getString("read_count")));
	...
	public void increaseReadCount( String article_id ) throws BoardException
	{
		...
		try{
			con = DriverManager.getConnection(dbUrl, dbUser, dbPass);
			String sql = "Update article set read_count = read_count + 1 where article_id = "+article_id;
			ps = con.prepareStatement(sql);
			ps.executeUpdate();
		}catch( Exception ex ){
